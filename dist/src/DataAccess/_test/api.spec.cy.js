describe('API', () => {
    ...
    it('get or create', () => {
        ...
        cy.window().then(async (wind) => {
            ...
            await Person.deleteAll();
            let [{ created, found }] = await Person.getOrCreate([{ userId: 2, username: 'kobe', email: 'kobe.bryant@lakers.com' }, { userId: 1, username: 'kobe', email: 'kobe.bryant@lakers.com' }]);
            expect(2).to.deep.equal(created.length);
        });
    });
    it('passes insert, passing null to auto incremente primary key', () => {
        cy.visit('./index.html');
        cy.window().should("have.property", "models");
        cy.window().then(async (wind) => {
            const Model = wind.models.Model;
            ...
            wind.models.register({
                databaseName: "auto-21",
                type: "localStorage",
                version: 1,
                models: [Person],
            });
            await Person.deleteAll();
            const [createdPerson, info] = await Person.create({ username: 'Peter', userId: null });
            expect(Object.assign({}, createdPerson)).to.deep.equal({ username: 'Peter', userId: createdPerson.userId }); // test fails
        });
    });
    it('passes edit something that doest not exist', () => {
        // should not pass in the feature
        cy.visit('./index.html');
        cy.window().should("have.property", "models");
        cy.window().then(async (wind) => {
            const Model = wind.models.Model;
            ...
            const [createdPerson, info] = await person.save({ username: 'Peter', userId: 1 });
            expect(createdPerson).to.deep.equal(1); // test fails
        });
    });
    it('update or create', () => {
        cy.visit('./index.html');
        ...
        cy.window().then(async (wind) => {
            ...
            await Person.deleteAll();
            let [{ created }] = await Person.updateOrCreate([{ userId: 2, username: 'kobe', email: 'kobe.bryant@lakers.com' }, { userId: 1, username: 'kobe', email: 'kobe.bryant@lakers.com' }]);
            expect(2).to.deep.equal(created.length);
            let [{ updated }] = await Person.updateOrCreate([{ userId: 2, username: 'kobe', email: 'kobe.bryant@lakers.com' }, { userId: 1, username: 'kobe', email: 'kobe.bryant@lakers.com' }]);
